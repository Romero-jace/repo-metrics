...
| Changing a JSON payload | The closing paragraphs of `## Adding a signal`, after its `###` subsection. Four payloads, five censuses: `internal/report/view.go` guarded by `degraded_test.go`, and `internal/report/repos.go`, `history.go` and `show.go` guarded by `payload_census_test.go`. |
